...
G2L.Main.Visible = false -- Set to false initially for the loading sequence
...
G2L.MinimizeBtn.Position = UDim2.new(1, -65, 0.5, -10)
...
Instance.new("UICorner", G2L.SearchBar).CornerRadius = UDim.new(0, 8)
...
G2L.SearchInput.FocusLost:Connect(function(enterPressed)
    ...
    task.spawn(function()
        local userId
        local success, _ = pcall(function() userId = Players:GetUserIdFromNameAsync(txt) end)
...
local btnDefs = {
    ...
    {name = "FreezeVictim", label = "FREEZE TRADE", sub = "freezes the trade", posY = 275},
...
for _, def in ipairs(btnDefs) do
    ...
    btnFrame.MouseButton1Click:Connect(function()
        ...
        toggleSound.SoundId = isEnabled and "rbxassetid://75311202481026" or "rbxassetid://17208361335"
...
local LoadFrame = Instance.new("Frame", G2L.Root)
LoadFrame.Size = UDim2.new(1, 0, 1, 0)
LoadFrame.BackgroundTransparency = 1

local Container = Instance.new("Frame", LoadFrame)
...
Icon.Image = "rbxassetid://97427877691946"
...
task.spawn(function()
    ...
    local pulse = TweenService:Create(Icon, pulseInfo, {Size = UDim2.new(0, 170, 0, 170)})
    pulse:Play()
    
    -- Subtle Glow Animation
    while LoadFrame.Parent do
        TweenService:Create(Icon, TweenInfo.new(1, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), {ImageColor3 = Color3.fromRGB(200, 200, 200)}):Play()
        task.wait(1)
        TweenService:Create(Icon, TweenInfo.new(1, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), {ImageColor3 = Color3.fromRGB(255, 255, 255)}):Play()
        task.wait(1)
    end
end)
...
task.spawn(function()
    while LoadFrame.Parent do
        for i = 0, 3 do
            Text.Text = "bypassing" .. string.rep(".", i)
            task.wait(0.5)
        end
    end
end)

task.wait(20) -- 20 Second Loading Time
